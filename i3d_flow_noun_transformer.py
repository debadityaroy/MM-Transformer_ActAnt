...
class EncoderTransformer(nn.Module):
    def __init__(self, embedding_length, verb_size, noun_size, attention_heads):
        ...
        self.flow_fc = nn.Linear(512, self.noun_size)
    ...
    def forward(self, obj_feat_seq, person_feat_seq, flow_feat_seq):
        ...
        #print(target.shape)
        noun_out = self.flow_decoder(target, flow_out)
        noun_out = self.flow_fc(noun_out).squeeze()
...
class TrainTest():
    ...
    def __init__(self, model, trainset, testset, criterion, batch_size, nepoch, ckpt_path):
        ...
        if os.path.exists(self.chkpath) == True:
            ...
            best_noun_acc = self.state['noun_acc']
            ...
            self.best_noun_acc = best_noun_acc
            self.start_epoch = start_epoch + 1
            self.model.to(device)                    
        else:
            self.best_noun_acc = -1.
            self.details = []   
            self.start_epoch = 0
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.testloader, 0):        
                ...
                noun_out = self.model(obj_feat_seq, person_feat_seq, flow_feat_seq)
                
                pred_noun_seq = torch.argmax(noun_out, dim=1)
                
                #print(pred_verb_seq)
                correct_noun = correct_noun + torch.sum(pred_noun_seq==target_noun_seq).item()                
                    
                loss = self.criterion(noun_out, target_noun_seq)
               
                print("\rIteration: {}/{},  Loss{}.".format(i+1, len(self.testloader), loss), end="")
                sys.stdout.flush()
                count += target_noun_seq.shape[0]
        return correct_noun/count*100, correct_noun    
    ...
    def train(self):        
        for epoch in range(self.start_epoch,self.nepoch):  
            ...
            TRAIN_NOUN_ACC = correct_noun/count*100
            TEST_NOUN_ACC, TEST_NOUN_COUNT = self.test()
            self.details.append((TRAIN_LOSS, TRAIN_NOUN_ACC, 0., TEST_NOUN_ACC))

            if TEST_NOUN_ACC > self.best_noun_acc:                
                self.state = {
                    'model': self.model.state_dict(),
                    'noun_acc': TEST_NOUN_ACC,
                    'epoch': epoch,
                    'details':self.details,            
                }        
                torch.save(self.state, self.chkpath)
                self.best_noun_acc = TEST_NOUN_ACC
                
            else:
                self.state['epoch'] = epoch
                torch.save(self.state, self.chkpath)
            elapsed_time = time.time() - start_time
            print('[{}] [{:.1f}] [Loss {:.3f}] [Noun Correct : {}] [Trn. Noun Acc {:.1f}] '.format(epoch, elapsed_time,
            TRAIN_LOSS, correct_noun, TRAIN_NOUN_ACC),end=" ")
            print('[Test Noun Cor {}] [Noun Acc {:.1f}]'.format(TEST_NOUN_COUNT, TEST_NOUN_ACC))             
...
ckpt_path = 'ckpt/i3d_flow_transformer_noun_1s.pt'
